...
import { TextureLoader, Vector2, AudioListener, AudioLoader, PMREMGenerator, CubeTextureLoader, WebGLMultisampleRenderTarget, RGBAFormat, sRGBEncoding, LoadingManager, Matrix4, Ray, Sphere as Sphere$1, Vector3, Plane as Plane$1, Vector4, PerspectiveCamera as PerspectiveCamera$1, LinearFilter, RGBFormat, WebGLRenderTarget, DepthTexture, DepthFormat, UnsignedShortType, Color, Box3, Raycaster, Camera, AnimationMixer, BufferGeometry, MeshPhysicalMaterial, MeshStandardMaterial, ShaderMaterial, UniformsUtils, ShaderChunk, Spherical, AdditiveBlending, PlaneBufferGeometry, Mesh, MeshDepthMaterial, Shape, BufferAttribute, RepeatWrapping } from 'three';
...
const useProgress = create$1(set => {
  ...
  loader.onStart = (item, loaded, total) => set({
    active: true,
    item,
    loaded,
    total,
  ...
  loader.onError = item => set(state => ({
    errors: [...state.errors, item]
  }));

  loader.onProgress = (item, loaded, total) => set({
    item,
    loaded,
    total,
    progress: loaded / total * 100
